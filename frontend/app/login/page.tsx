...
async function getLoginClientContext() {
  ...
  const highEntropy: HighEntropyClientHints =
...
export default function LoginPage() {
  ...
  async function handleRequestReset() {
    ...
    try {
      ...
      setResetMessage(
        result.message || "Permintaan reset PIN dikirim ke admin.",
      );
    } catch (error) {
      setResetMessage(
        error instanceof Error
          ? error.message
          : "Gagal mengirim permintaan reset PIN.",
      );
    } finally {
      ...
      <div className="glass-card relative w-full max-w-5xl overflow-hidden rounded-[2rem] border border-white/70">
        <div className="grid md:grid-cols-2">
          <section className="hidden bg-[linear-gradient(160deg,#1e3a8a,#2563eb,#06b6d4)] p-10 text-white md:block">
            <p className="rounded-full bg-white/20 px-4 py-1 text-xs font-semibold uppercase tracking-[0.2em]">
              KasRT Warga
            </p>
            ...
            <p className="mt-4 max-w-sm text-sm text-blue-100">
              Pantau kontribusi, approval, dan aktivitas jimpitan dari satu
              dashboard modern.
            </p>
          </section>
          ...
          <section className="p-6 sm:p-10">
            ...
            <p className="mt-2 text-sm text-[var(--text-muted)]">
              Gunakan nomor HP dan PIN Anda.
            </p>
            <p className="mt-1 text-sm text-[var(--text-muted)]">
              PIN awal ketik 123456
            </p>
            ...
            <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
              ...
              {showReset ? (
                <div className="rounded-2xl border border-amber-200 bg-amber-50/80 px-4 py-3 text-sm text-amber-900">
                  <p className="font-semibold">Reset PIN ke default</p>
                  <p className="mt-1 text-xs leading-5">
                    Pastikan nomor HP di atas sudah benar. Panduan akan dikirim
                    ke nomor HP, lalu PIN Anda diatur ke default (pin awal).
                  </p>
                  <Button
                    type="button"
                    variant="ghost"
                    className="mt-3 w-full bg-white/70 text-amber-900"
                    onClick={handleRequestReset}
                    disabled={resetLoading}
                  >
                    {resetLoading ? "Mengirim..." : "Reset PIN"}
                  </Button>
                  {resetMessage ? (
                    <p className="mt-2 rounded-xl border border-amber-200 bg-white/70 px-3 py-2 text-xs font-semibold text-amber-900">
                      {resetMessage}
                    </p>
                  ) : null}
                </div>
